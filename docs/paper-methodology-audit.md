...
| One Lloyd-Max scalar codebook is built offline per input dimension and bit width. | Pass | `get_codebook(dim, bits)` in `src/orbitquant/codebooks/lloyd_max.py`; `tests/test_codebooks.py`; `tests/test_orbit_linear.py`; `tests/test_artifact_writer.py` | Persistent cache keys include algorithm version, dimension, and bits. Artifact codebook tensors are deduplicated by dimension and bits, not by layer. No prompt or timestep identifier enters the codebook. |
