...
def test_native_packed_matmul_kernel_package_stays_kernel_builder_abi3_compliant():
    ...
    assert cuda_source.count("C10_CUDA_KERNEL_LAUNCH_CHECK();") == 14
    ...
    assert "orbitquant_quantize_rows_int8_kernel" in cuda_source
    assert "orbitquant_int8_rows_gemv_kernel" in cuda_source
